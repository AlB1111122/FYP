...
void createApp(video_app *self) {
  ...
  self->surface_rgb = SDL_CreateRGBSurfaceWithFormat(0, WIN_WIDTH, WIN_HEIGHT,
                                                     24, SDL_PIXELFORMAT_RGB24);
  if (!self->surface_rgb) {
    std::cout << "Error: Couldn't create surface. SDL_Error: \n"
              << SDL_GetError();
    self->wants_to_quit = true;
  }

  SDL_BlitSurface(self->surface_rgb, NULL, SDL_GetWindowSurface(self->window),
                  NULL);
...
void updateFrame(plm_t *mpeg, plm_frame_t *frame, void *user) {
  ...
  memcpy(self->surface_rgb->pixels, new_rgb_data, N_PIXELS);
...
void updateVideo(video_app *self) {
  ...
  if (elapsed_time > (frame_rate_info.frame_ms * 2)) {
    self->slow_flag = true;
    if (self->slow_flag == true) {
      // 2 slow frames in a row cause a skip
      std::cout << "tripped"
                << "\n";
      seek_to = plm_time +
                ((elapsed_time - (frame_rate_info.frame_ms * 1.2)) / 1000.0);
    }
  }
  if (seek_to != -1) {
    plm_seek(self->plm, seek_to, TRUE);  // functional but slower than nessesary
...
void destroy(video_app *self) {
  SDL_FreeSurface(self->surface_rgb);
