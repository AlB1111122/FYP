...
double Timer::to_sec(uint64_t time) {
  ...
  return (double)division_result.quot +
         ((double)division_result.rem / (double)this->SYS_TIMER_Hz);
}

double Timer::to_milli(uint64_t time) {
  ...
  lldiv_t division_result = lldiv(time, (this->SYS_TIMER_Hz / 1000));
  return (double)division_result.quot +
         ((double)division_result.rem / (this->SYS_TIMER_Hz / 1000));
}
