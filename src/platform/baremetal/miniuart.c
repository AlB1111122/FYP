...
void uart_init() {
  ...
  mmio_write(AUX_MU_IER_REG, 0);   // disable interupts
  mmio_write(AUX_MU_CNTL_REG, 0);  // disable uart recive and disable
  // 8 bits mode,(error in datasheet: says ony 0 matters but false need 3)
  mmio_write(AUX_MU_LCR_REG, 3);
  mmio_write(AUX_MU_MCR_REG, 0);  // UART1_RTS line to high
  mmio_write(AUX_MU_IER_REG, 0);  // disable interrupts(again??)
