...
FrameBuffer::FrameBuffer()
{
    ...
    mbox->set_phy_wh_tag = MBOX_TAG_SETPHYWH;
    mbox->set_phy_wh_size = 8;
    mbox->set_phy_wh_value = 0;
    mbox->width = 1920;
    mbox->height = 1080;
    
    mbox->set_virt_wh_tag = MBOX_TAG_SETVIRTWH;
    mbox->set_virt_wh_size = 8;
    mbox->set_virt_wh_value = 8;
    mbox->virt_width = 1920;
    mbox->virt_height = 1080;
    
    mbox->set_virt_off_tag = MBOX_TAG_SETVIRTOFF;
    mbox->set_virt_off_size = 8;
    mbox->set_virt_off_value = 8;
    mbox->x_offset = 0;
    mbox->y_offset = 0;
    
    mbox->set_depth_tag = MBOX_TAG_SETDEPTH;
    mbox->set_depth_size = 4;
    mbox->set_depth_value = 4;
    mbox->depth = 32;
    
    mbox->set_pixel_order_tag = MBOX_TAG_SETPXLORDR;
    mbox->set_pixel_order_size = 4;
    mbox->set_pixel_order_value = 4;
    mbox->pixel_order = 1;
    
    mbox->get_fb_tag = MBOX_TAG_GETFB;
    mbox->get_fb_size = 8;
    mbox->get_fb_value = 8;
    mbox->fb_pointer = 4096;
    mbox->fb_size = 0;
    
    mbox->get_pitch_tag = MBOX_TAG_GETPITCH;
    mbox->get_pitch_size = 4;
    mbox->get_pitch_value = 4;
    mbox->pitch = 0;
    
    mbox->end_tag = MBOX_TAG_LAST;

    // Check call is successful and we have a pointer with depth 32
    if (FB_mailbox.mailboxWriteRead(MBOX_CH_PROP) && mbox->depth == 32 && mbox->fb_pointer != 0) {
        mbox->fb_pointer &= 0x3FFFFFFF; // Convert GPU address to ARM address
